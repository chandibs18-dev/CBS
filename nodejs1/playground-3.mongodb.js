...
// Find all users sorted by age (ascending)
const usersByAge = db.getCollection(collection).find().sort({ age: 1 }).toArray();

console.log("Users sorted by age (ascending):", usersByAge.map(u => ({ name: u.name, age: u.age })));

const ageRanges = db.getCollection(collection).aggregate([
    {
      $bucket: {
        groupBy: "$age",
        boundaries: [0, 20, 30, 40, 50], // adjust ranges as needed
        default: "50+",
        output: {
          totalUsers: { $sum: 1 },
          names: { $push: "$name" } // optional: list names in that range
        }
      }
    }
  ]).toArray();
  
  console.log("Users per age range:", ageRanges);

  const italyUsers = db.getCollection("slttags").find({ "company.location.country": "Italy" }).toArray();
  const italyCount = italyUsers.length;
  
  ({ count: italyCount, users: italyUsers });  

  const appleGreenFemaleCount = db.getCollection("slttags").find({
    favoriteFruit: "apple",
    eyeColor: "green",
    gender: "female"
  }).count();
  
  appleGreenFemaleCount;
